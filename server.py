    ...
    def sendData(self):
        while True:
            if not que.empty():
                ...
                print('users',end=':')
                print(users)
                # message[0]:地址('127.0.0.1', 6177) message[1]:信息内容'Im test:;test1:;jluzh'
                #users是一个列表，里面存储各个user元组，users[i]表示连接的对象,user[0]表示socket信息，user[1]表示用户名，user[2]表示地址
                #user[1]: 'jluzh', user[2]: ('127.0.0.1', 6002)
                if isinstance(message[1], str):                   # 如果data是str则返回Ture
                    for i in range(len(users)):
                        # user[i][1]是用户名, users[i][2]是addr, 将message[0]改为用户名
                        for j in range(len(users)):
                            # 匹配这个信息的来源是哪个用户，如果信息的来源地址==当前用户的地址
                            if message[0] == users[j][2]:
                                print(' this: message is from user[{}]'.format(j))
                                if '@Robot' in message[1] and reply_text == '':
                                    #这里调用robot api
                                    msg = message[1].split(':;')[0] # 这是信息的内容
